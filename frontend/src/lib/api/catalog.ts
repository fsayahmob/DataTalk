...
export async function generateCatalog(): Promise<CatalogGenerateResponse | null> {
  try {
    const res = await apiFetch(`${API_BASE}/catalog/generate`, { method: "POST" });
    const data = await res.json();
    return data;
  } catch (e) {
...
export async function enrichCatalog(tableIds: number[]): Promise<CatalogEnrichResponse | null> {
  try {
    const res = await apiFetch(`${API_BASE}/catalog/enrich`, {
      method: "POST",
...
export async function deleteCatalog(): Promise<boolean> {
  try {
    const res = await apiFetch(`${API_BASE}/catalog`, { method: "DELETE" });
    return res.ok;
  } catch (e) {
...
export async function toggleTableEnabled(
  ...
  try {
    const res = await apiFetch(`${API_BASE}/catalog/tables/${tableId}/toggle`, {
      method: "PATCH",
    });
...
export async function updateColumnDescription(columnId: number, description: string): Promise<boolean> {
  try {
    const res = await apiFetch(`${API_BASE}/catalog/columns/${columnId}/description`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
...
export async function fetchLatestRun(): Promise<RunResponse> {
  try {
    const res = await apiFetch(`${API_BASE}/catalog/latest-run`);
    if (!res.ok) {
      if (res.status === 404) {
...
export async function fetchRun(runId: string): Promise<RunResponse> {
  try {
    const res = await apiFetch(`${API_BASE}/catalog/run/${runId}`);
    if (!res.ok) {
      if (res.status === 404) {
...
export async function fetchCatalogJobs(limit = 50): Promise<CatalogJob[]> {
  try {
    const res = await apiFetch(`${API_BASE}/catalog/jobs?limit=${limit}`);
    const data = await res.json();
    return data.jobs || [];
  } catch (e) {
...
export async function fetchRuns(): Promise<Run[]> {
  try {
    const res = await apiFetch(`${API_BASE}/catalog/runs`);
    if (!res.ok) return [];
    const data = await res.json();
    return data.runs || [];
  } catch (e) {
...
/**
 * Retry a failed job by resetting it to pending status and relaunching Celery task.
 * @param jobId The job ID to retry
 * @returns The reset job with task_id, or null on error
 */
export async function retryJob(jobId: number): Promise<{ job: CatalogJob; task_id: string; message: string } | null> {
  try {
    const res = await apiFetch(`${API_BASE}/catalog/jobs/${jobId}/retry`, {
      method: "POST",
    });
    if (!res.ok) {
      const error = await res.json();
      console.error("Erreur retry job:", error.detail);
      return null;
    }
    return await res.json();
  } catch (e) {
    console.error("Erreur retry job:", e);
    return null;
  }
}
